...
void ellipticSolveSetup(elliptic_t *elliptic, dfloat lambda, occa::properties &kernelInfo){
  ...
  if(Nbytes>0){
    occa::memory o_sendBuffer = mesh->device.malloc(Nbytes, NULL,"mapped: true");
    occa::memory o_recvBuffer = mesh->device.malloc(Nbytes, NULL,"mapped: true");

    elliptic->sendBuffer = (dfloat*) occa::opencl::getCLMappedPtr( o_sendBuffer);
    elliptic->recvBuffer = (dfloat*) occa::opencl::getCLMappedPtr( o_recvBuffer);

    occa::memory o_gradSendBuffer = mesh->device.malloc(2*Nbytes, NULL,"mapped: true");
    occa::memory o_gradRecvBuffer = mesh->device.malloc(2*Nbytes, NULL,"mapped: true");

    elliptic->gradSendBuffer = (dfloat*) occa::opencl::getCLMappedPtr( o_gradSendBuffer);
    elliptic->gradRecvBuffer = (dfloat*) occa::opencl::getCLMappedPtr( o_gradRecvBuffer);
  }else{
